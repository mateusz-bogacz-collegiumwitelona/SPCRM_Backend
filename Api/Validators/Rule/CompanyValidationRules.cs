...
namespace Api.Validators.Rule
{
    public static class CompanyValidationRules
    {
        ...
        public static IRuleBuilderOptions<T, string?> ApplyCompanyStreetRules<T>(this IRuleBuilder<T, string?> ruleBuilder)
            => ruleBuilder
                .NotEmpty().WithErrorCode(ErrorCodes.StreetRequired)
                .MaximumLength(200).WithErrorCode(ErrorCodes.StreetLengthInvalid);

        public static IRuleBuilderOptions<T, string?> ApplyCompanyCityRules<T>(this IRuleBuilder<T, string?> ruleBuilder)
            => ruleBuilder
                .NotEmpty().WithErrorCode(ErrorCodes.CityRequired)
                .MaximumLength(100).WithErrorCode(ErrorCodes.CityLengthInvalid);

        public static IRuleBuilderOptions<T, string?> ApplyCompanyNipRules<T>(this IRuleBuilder<T, string?> ruleBuilder)
            => ruleBuilder
                .NotEmpty().WithErrorCode(ErrorCodes.NipRequired)
                .Must(BeValidNipChecksum).WithErrorCode(ErrorCodes.NipNotValid);

        public static IRuleBuilderOptions<T, string?> ApplyCompanyZipCodeRules<T>(this IRuleBuilder<T, string?> ruleBuilder)
            => ruleBuilder
